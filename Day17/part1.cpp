...
namespace std
{
    ...
    struct hash<Point>
    {
        ...
        {
            return hash<int>()(obj.getX() * obj.getY() * obj.getZ());
        }
    };
} // namespace std
...
void addPoint(unordered_set<Point> points, unordered_set<Point> &tmp, Point &p)
{
    if (tmp.find(p) == tmp.end())
    {
        ...
        for(size_t j = 0; j < offsets.size(); j++) {
            if (points.find(Point(p.getX() + offsets[j][0], p.getY() + offsets[j][1], p.getZ() + offsets[j][2])) 
                != points.end())
                numActNei++;
...
int main()
{
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        {
            int x = it->getX(), y = it->getY(), z = it->getZ();

            for(size_t j = 0; j < offsets.size(); j++){
                Point p(x + offsets[j][0], y + offsets[j][1], z + offsets[j][2]);
